...
function calcularExpiracionReserva(inicioIso) {
  ...
  return new Date(
    inicio.getTime() + RESERVATION_DURATION_HOURS * 60 * 60 * 1000
  );
...
async function reservarBoletas(
  sheets,
  sheetId,
  tabName,
  boletas,
  ...
  console.log(
    `🔒 Reservando ${boletas.length} boletas para ${clienteWaId}`
  );
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (boletasSet.has(numero)) {
        ...
        updates.push({
          ...
          values: [["RESERVADA", clienteWaId, inicioReserva]]
        });
    ...
    console.error("❌ Error reservando boletas:", error);
...
async function liberarReservasExpiradas(
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (estado === "RESERVADA" && reservaTsStr) {

        const expiracion =
          calcularExpiracionReserva(reservaTsStr);

        if (now > expiracion) {

          console.log(
            `♻️ Liberando boleta ${rows[i][0]}`
          );
    ...
    if (updates.length > 0) {
      ...
      console.log(
        `✅ ${updates.length} reservas liberadas`
      );
...
async function reservarMultiplesBoletas(
  sheetsRepository,
  cantidad,
  wa_id
) {
...
async function getBoletasReservadasCliente(
  sheetsRepository,
  wa_id
) {
...
async function reservarBoletaSegura(
  sheets,
  sheetId,
  tabName,
  boleta,
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (numero === boleta) {
      ...
      if (estado !== "DISPONIBLE") {
        ...
        console.log(`⚠️ Boleta ${boleta} ya no disponible`);
